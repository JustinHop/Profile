...
def main():
    ...
    parser.add_option('-m', '--mute', action='store_true', dest='mute', default=False, help='mute the main audio channel')
    ...
    if option.percent or option.mute:
        ...
        raise SystemExit(AlsaMixer(option.pcm, option.mute, option.percent).label)
    # OSS code
    # if option.percent or option.mute:
    #     if option.mute:
    #         if not option.pcm:
    ...
    #     if option.quiet:
    #         raise SystemExit(AlsaMixer(option.pcm, True, option.percent).label)
    #     volume = AlsaMixer(option.pcm, True, option.percent)
    elif option.status:
